// app/calculator/CalculatorForm.tsx
"use client";
...
export default function CalculatorForm() {
  ...
  const [potencies, setPotencies] = useState<Potencies>({
    alk: {}, ca: {}, mg: {}
  });
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      for (const k of paramKeys) {
        ...
        if (mode === "perLiter") {
          if (perL !== undefined && tankLiters !== undefined) {
            incPerMlTank = perL * tankLiters;
          }
        } else {
  ...
  async function autofillFromPreferred() {
    try {
      setLoadingPref(true);
      // Pick user's most recent tank, then preferred_products for alk,ca,mg
      const { data: tanks, error: tErr } = await sb
        .from("tanks")
      ...
      for (const row of prefs || []) {
        ...
        // per liter potency: (delta / dose) / volume_liters
        const perLiter =
      ...
      for (const p of out) {
        if (p.perLiter != null) {
          next[p.parameter_key].perLiter = p.perLiter;
        }
      }
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border p-4">
        <h2 className="text-lg font-semibold mb-2">Tank & Mode</h2>
      ...
      <section className="rounded-2xl border p-4">
        ...
        <p className="text-sm text-muted-foreground mb-3">
          ...
          If you chose per ml per litre, we will multiply by your tank size.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      ...
      <section className="rounded-2xl border p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" inputMode="decimal" className="w-full border rounded-lg p-2 bg-background"
              value={current.alk ?? ""}
              onChange={(e) => setCurrent({ ...current, alk: safeNum(e.target.value) })} placeholder="e.g. 6.7" />
          </div>
          <div>
            <label className="block text-sm text-muted-foreground mb-1">Current Ca (ppm)</label>
            <input type="number" inputMode="decimal" className="w-full border rounded-lg p-2 bg-background"
              value={current.ca ?? ""}
              onChange={(e) => setCurrent({ ...current, ca: safeNum(e.target.value) })} placeholder="e.g. 410" />
          </div>
          <div>
            <label className="block text-sm text-muted-foreground mb-1">Current Mg (ppm)</label>
            <input type="number" inputMode="decimal" className="w-full border rounded-lg p-2 bg-background"
              value={current.mg ?? ""}
              onChange={(e) => setCurrent({ ...current, mg: safeNum(e.target.value) })} placeholder="e.g. 1320" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
          <div>
            ...
            <input type="number" inputMode="decimal" className="w-full border rounded-lg p-2 bg-background"
              value={target.alk ?? ""}
              onChange={(e) => setTarget({ ...target, alk: safeNum(e.target.value) })} placeholder="e.g. 8.0" />
          </div>
          <div>
            <label className="block text-sm text-muted-foreground mb-1">Target Ca (ppm)</label>
            <input type="number" inputMode="decimal" className="w-full border rounded-lg p-2 bg-background"
              value={target.ca ?? ""}
              onChange={(e) => setTarget({ ...target, ca: safeNum(e.target.value) })} placeholder="e.g. 430" />
          </div>
          <div>
            <label className="block text-sm text-muted-foreground mb-1">Target Mg (ppm)</label>
            <input type="number" inputMode="decimal" className="w-full border rounded-lg p-2 bg-background"
              value={target.mg ?? ""}
              onChange={(e) => setTarget({ ...target, mg: safeNum(e.target.value) })} placeholder="e.g. 1400" />
          </div>
        </div>
      </section>
          ...
          {(["alk","ca","mg"] as const).map((k) => (
            <div key={k} className="border rounded-xl p-3">
              ...
              <div className="text-xs mt-1">
                Change: {round2(deltaDose[k]) || "—"} ml/day
              </div>
            </div>
          ))}
        </div>
