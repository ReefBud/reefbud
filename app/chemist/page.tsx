...
export default function Chemist() {
  ...
  const [targets, setTargets] = useState<Record<ParamKey, number>>({ alk: 8.3, ca: 430, mg: 1350, po4: 0.05, no3: 10 });
  const [prod, setProd] = useState<Record<ParamKey, string|undefined>>({ alk: undefined, ca: undefined, mg: undefined, po4: undefined, no3: undefined });
  ...
  useEffect(() => {
    (async () => {
      ...
      // load targets
      const { data: params } = await supabase.from("parameters").select("*");
      const map: Record<number, ParamKey> = {};
      params?.forEach((p:any) => { map[p.id] = p.key; });
      const { data: tgs } = await supabase.from("targets").select("parameter_id, target_value").eq("tank_id", tk.id);
  ...
  async function saveTarget(key: ParamKey, value: number) {
    ...
    if (!p) return;
    await supabase.from("targets").upsert({ tank_id: tank.id, parameter_id: p.id, target_value: value }).select();
  }
  ...
  return (
    <main className="space-y-6">
      ...
      {(!userId || !tank) ? <div className="card">Sign in to manage targets.</div> : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                  ...
                  <input className="input" type="number" step="0.001"
                    value={targets[key]}
                    onChange={e=>{ const v = Number(e.target.value); setTargets(s=>({ ...s, [key]: v })); saveTarget(key, v); }} />
                </div>
